...
#SBATCH -J cif_scenario1#job name
#SBATCH -o cif_scenario1-%A-%a.out #specify stdout output file (%A expands to array jobID, %a expands to array task id)
#SBATCH -a 1-6
...
if [ $SLURM_ARRAY_TASK_ID == 1 ]; then
	Rscript sim_experiment_1.R 500 100 kmedians
elif [ $SLURM_ARRAY_TASK_ID == 2 ]; then
	Rscript sim_experiment_1.R 1000 100 kmedians
elif [ $SLURM_ARRAY_TASK_ID == 3 ]; then
	Rscript sim_experiment_1.R 1500 100 kmedians
elif [ $SLURM_ARRAY_TASK_ID == 4 ]; then
	Rscript sim_experiment_1.R 500 50 kmedians
elif [ $SLURM_ARRAY_TASK_ID == 5 ]; then
	Rscript sim_experiment_1.R 1000 50 kmedians
elif [ $SLURM_ARRAY_TASK_ID == 6 ]; then
	Rscript sim_experiment_1.R 1500 50 kmedians
fi
